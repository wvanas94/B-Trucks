//
//  UpdateOwnerInfo.swift
//  B-Trucks
//
//  Created by Wesley Van As on 3/28/17.
...
class updateOwnerInfo: UIViewController {
    ...
    @IBAction func submit(_ sender: AnyObject) {
        ...
        if fNameValidated && lNameValidated {
            print("Validated!")
            let myUrl = URL(string: "http://cgi.soic.indiana.edu/~team18/updateOwner.php")
                        ...
                        OperationQueue.main.addOperation {
                            ...
                            let cancelAction = UIAlertAction(title: "OK",
                                                             style: .cancel, handler: {
                                                                (_)in
                                                                self.performSegue(withIdentifier: "ownerSettingsToLogin", sender: self)
                            })
                            alert.addAction(cancelAction)
                            self.present(alert, animated: true)
                        }
